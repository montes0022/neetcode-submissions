class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        total = 0
        max_in_pile = 0
        #manual for loop to find max in piles, max(piles) works too.
        ...
        while l <= r:
            ...
            if hours <= h:
                #because you want to find the smallest, we update result here.
                result = min(result, k)
                r = k - 1 #look for a smaller k, left portion
            else:
                #look for a bigger k, right portion, means the k too small/slow
                l = k + 1
