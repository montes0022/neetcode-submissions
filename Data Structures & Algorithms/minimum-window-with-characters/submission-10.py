class Solution:
    def minWindow(self, s: str, t: str) -> str:
        ...
        for r in range(len(s)):
            ...
            if s[r] in counts and countt[s[r]] == counts[s[r]]:
                HAVE += 1
            ...
            #since we want to move l up, while have and need are equal
            #we want to make them not equal so we can attempt to find
            #a smaller string. this is the sliding window technique.
            #like the first sliding window problem, if we did an if statement
            #to check have and need, we could not move l up many times if 
            #needed. remember, l is something that r has already pointed
            #to. l can either go up one, or four, we need a while loop to
            #accomplish that.
            while HAVE == NEED:
                #update your result.
                ...
                #AND
                #if the value of the key at countt[s[l]] 
                #is smaller than counts[s[l]]...
                #move HAVE down because we no longer satisfy that conditon.
                if s[l] in counts and countt[s[l]] < counts[s[l]]:
                    HAVE -= 1
        ...
        #also known as python unpacking
        #l = res[0], r = res[1]
        l, r = res
